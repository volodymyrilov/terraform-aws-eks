...
echo "${null_resource.update_config_map_aws_auth.triggers.kube_config_map_rendered}" > ${path.module}/kube_config.yaml & \
echo "${null_resource.update_config_map_aws_auth.triggers.config_map_rendered}" > ${path.module}/aws_auth_configmap.yaml & \
kubectl apply -f ${path.module}/aws_auth_configmap.yaml --kubeconfig ${path.module}/kube_config.yaml && break || \
sleep 10; \
done; \
rm ${path.module}/aws_auth_configmap.yaml ${path.module}/kube_config.yaml;
EOS
